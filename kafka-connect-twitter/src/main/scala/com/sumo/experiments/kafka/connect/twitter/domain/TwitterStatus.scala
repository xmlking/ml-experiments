...
object TwitterUser {
  ...
  val schema = SchemaBuilder.struct().name("com.sumo.experiments.kafka.connect.twitter.domain.TwitterUser")
...
object TwitterStatus {
  ...
  val schema = SchemaBuilder.struct().name("com.sumo.experiments.kafka.connect.twitter.domain.TwitterStatus")
    .field("id", Schema.INT64_SCHEMA)
